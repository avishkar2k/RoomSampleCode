...
public class HomeActivity extends AppCompatActivity implements ApiCallbackListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        viewModel.getAllRestaurantsList().observe(this, new Observer<List<RestaurantEntity>>() {
            @Override
            public void onChanged(@Nullable List<RestaurantEntity> restaurantEntities) {
                ...
                if (restaurantEntities != null) {
                    if (!restaurantEntityList.isEmpty()) restaurantEntityList.clear();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String permissions[], @NonNull int[] grantResults) {
        switch (requestCode) {
            case PLACE_PICKER_REQUEST:
                if (!AppUtils.getInstance().hasRequiredPermission(this)) {
                    boolean showRationaleAccessFineLocation = shouldShowRequestPermissionRationale(Manifest.permission.ACCESS_FINE_LOCATION);
                    boolean showRationaleAccessCoarseLocation = shouldShowRequestPermissionRationale(Manifest.permission.ACCESS_COARSE_LOCATION);
